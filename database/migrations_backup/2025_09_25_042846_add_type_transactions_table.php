...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (DB::connection()->getDriverName() === 'pgsql') {
            ...
            DB::statement("ALTER TABLE transactions ADD CONSTRAINT transactions_type_check CHECK (type IN ('profit', 'deposit', 'withdrawal', 'commission', 'roi', 'investment', 'bonus', 'adjust'))");
        } else {
    ...
    public function down(): void
    {
        if (DB::connection()->getDriverName() === 'pgsql') {
            DB::statement("ALTER TABLE transactions DROP CONSTRAINT IF EXISTS transactions_type_check");
            DB::statement("ALTER TABLE transactions ADD CONSTRAINT transactions_type_check CHECK (type IN ('profit', 'deposit', 'withdrawal', 'commission', 'roi', 'investment', 'bonus'))");
        } else {
